...
const compile = () => {
  ...
  console.log(`Compiling static HTML files for ${blogs.length} blogs...`);

  // Sort blogs descending by date
  const sortedBlogs = [...blogs].sort((a, b) => new Date(b.date) - new Date(a.date));

  blogs.forEach(blog => {
    const { title, slug, description, keywords, category, readTime, imageUrl, bodyContent, date, tags, faqs } = blog;
    ...
    // 1. Generate Dynamic Recent Posts Sidebar (10 most recent posts)
    const recentPostsList = sortedBlogs.slice(0, 10).map(b => {
      const isActive = b.slug === slug;
      return `<li class="${isActive ? 'active' : ''}"><a href="${b.slug}.html">${b.title}</a></li>`;
    }).join('\n            ');

    // 2. Generate Dynamic Related Posts Section (Up to 4 related articles)
    const candidates = blogs.filter(b => b.slug !== slug);
    const scoredCandidates = candidates.map(b => {
      let score = 0;
      if (b.category === category) score += 5;
      if (b.tags && tags && Array.isArray(b.tags) && Array.isArray(tags)) {
        const commonTags = b.tags.filter(t => tags.includes(t));
        score += commonTags.length * 2;
      }
      return { blog: b, score };
    });
    scoredCandidates.sort((a, b) => b.score - a.score || new Date(b.blog.date) - new Date(a.blog.date));
    const relatedCardsMarkup = scoredCandidates.slice(0, 4).map(s => {
      const b = s.blog;
      return `
            <div class="related-card" style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; padding: 20px; display: flex; flex-direction: column; gap: 12px; transition: all 0.3s ease; box-shadow: var(--shadow-card);">
              <img src="/${b.imageUrl}" alt="${b.title.replace(/"/g, '&quot;')}" style="width: 100%; height: 130px; object-fit: cover; border-radius: 12px; border: 1px solid var(--border-color);" />
              <span style="font-size: 11px; font-weight: 700; color: var(--accent-primary); text-transform: uppercase; letter-spacing: 0.05em;">${b.category}</span>
              <a href="/${b.slug}.html" style="font-size: 14px; font-weight: 700; color: var(--text-main); line-height: 1.4; text-decoration: none; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; height: 38px;">${b.title}</a>
            </div>
      `;
    }).join('\n            ');

    // 3. Generate JSON-LD Schema (BlogPosting, FAQPage, BreadcrumbList)
    ...
    const structuredData = {
      ...
      "@graph": [
        {
          "@type": "BlogPosting",
          "@id": `https://www.doppelganger.world/${slug}.html#article`,
          "isPartOf": {
            "@type": "WebPage",
          ...
          "headline": escTitle,
          "image": [
            `https://www.doppelganger.world/${imageUrl}`
          ],
        ...
        {
          "@type": "BreadcrumbList",
          "@id": `https://www.doppelganger.world/${slug}.html#breadcrumb`,
          "itemListElement": [
            {
              "@type": "ListItem",
              "position": 1,
              "name": "Home",
              "item": "https://www.doppelganger.world/"
            },
            {
              "@type": "ListItem",
              "position": 2,
              "name": "Blog",
              "item": "https://www.doppelganger.world/blog"
            },
            {
              "@type": "ListItem",
              "position": 3,
    ...
    if (faqSchemaItems.length > 0) {
      structuredData["@graph"].push({
        "@type": "FAQPage",
        "@id": `https://www.doppelganger.world/${slug}.html#faq`,
...
<head>
  ...
  <meta property="og:image" content="https://www.doppelganger.world/${imageUrl}" />
  ...
  <meta name="twitter:image" content="https://www.doppelganger.world/${imageUrl}" />
  ...
  <link rel="stylesheet" href="/ad-styles.css?v=2.0.0" />
  <link rel="stylesheet" href="/cookie-consent.css?v=2.0.0" />
  <!-- Google AdSense -->
...
<body>
    ...
    <div style="display: flex; gap: 30px; flex-wrap: wrap;">
      ...
      <article style="flex: 2.5; min-width: 320px; background: var(--bg-card); padding: 32px; border-radius: 20px; border: 1px solid var(--border-color); box-shadow: var(--shadow-card);">
        ...
        <div class="faqs-section" style="margin-top: 40px; border-top: 1px solid var(--border-color); padding-top: 32px;">
          ...
          ${faqs.map(f => `
            <div class="faq-item">
              <div class="faq-question">Q: ${f.q}</div>
  ...
  <script src="/chat.js?v=2.0.0" defer></script>
  <script src="/ad-components.js?v=2.0.0" defer></script>
  <script src="/cookie-consent.js?v=2.0.0" defer></script>
  ...
  <script>
    ...
    console.log(`Compiled: ${slug}.html`);
  });

  console.log("🎉 Static compilation complete!");
};
